...
import { ShoppingBag, Heart, ChevronLeft, Loader2, Truck, ShieldCheck, RefreshCw } from 'lucide-react';
...
const ProductDetail = () => {
  ...
  if (error || !product) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen space-y-4 bg-[#FCF9F6]">
        <p className="font-serif italic text-stone-500">Masterpiece not found.</p>
        <button 
  ...
  const handleAddToCart = () => {
    addItem(product.id, 1); // Add 1 item to cart
  };
          ...
          <div className="w-full lg:w-3/5 flex flex-col-reverse md:flex-row gap-4">
                ...
                {product.images.map((imgUrl, idx) => (
                  <button
                    ...
                    className={`w-20 aspect-[3/4] shrink-0 overflow-hidden border-2 transition-all ${
                      selectedImage === idx ? 'border-amber-800' : 'border-transparent opacity-50'
                    }`}
                  >
            ...
            <div className="flex-1 aspect-[4/5] overflow-hidden bg-stone-100 rounded-sm h-[500px]">
              {product.images?.length > 0 ? (
                ...
                <div className="flex items-center justify-center h-full text-stone-400 italic font-serif">Image Unavailable</div>
              )}
            </div>
          </div>
          ...
          <div className="w-full lg:w-2/5 space-y-10 py-4">
            ...
            <div className="space-y-4">
              ...
              <p className="text-[10px] text-center text-stone-400 uppercase tracking-widest">
                {product.stockQuantity > 0 ? `Limited Edition: ${product.stockQuantity} Remaining` : 'Being Woven'}
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 pt-10 border-t border-stone-200">
              <div className="text-center space-y-2">
                <Truck size={18} strokeWidth={1} className="mx-auto text-amber-800" />
                <p className="text-[9px] uppercase tracking-tighter text-stone-500">Global Shipping</p>
              </div>
              <div className="text-center space-y-2">
                <ShieldCheck size={18} strokeWidth={1} className="mx-auto text-amber-800" />
                <p className="text-[9px] uppercase tracking-tighter text-stone-500">Pure Handloom</p>
              </div>
              <div className="text-center space-y-2">
                <RefreshCw size={18} strokeWidth={1} className="mx-auto text-amber-800" />
                <p className="text-[9px] uppercase tracking-tighter text-stone-500">Easy Returns</p>
              </div>
            </div>
          </div>
